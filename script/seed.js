...
async function seed() {
    ...
    try{
        ...
        for (let i = 0; i < 10; i++){
            ...
            await Layers.create({frameId: newFrame.id, pixels: JSON.stringify(grid1), layerOrder: 1})
            await Layers.create({frameId: newFrame.id, pixels: JSON.stringify(grid2), layerOrder: 2})
            let newFrame2 = await Frames.create({spriteId: newSprite.id, frameOrder: 2})
            await Layers.create({frameId: newFrame2.id, pixels: JSON.stringify(grid1), layerOrder: 1})
            await Layers.create({frameId: newFrame2.id, pixels: JSON.stringify(grid2), layerOrder: 2})
            }
    } catch(error){
        console.log(error)
    }
